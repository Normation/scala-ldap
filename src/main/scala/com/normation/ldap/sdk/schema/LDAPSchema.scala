  ...
  def +=(oc:LDAPObjectClass) : LDAPSchema = {
    ...
    ocs.get(pKey) match {
      ...
      case Some(p) => ocs.get(key) match {
        case Some(x) if(x != oc) => throw new TechnicalException(s"""Can not register object class %s because an other different object class with same name was already registerd. 
                                     | existing: ${oc.name}
                                     | new     : $oc""".stripMargin('|'))
        case _ => {
          ocs += ((key,oc))
